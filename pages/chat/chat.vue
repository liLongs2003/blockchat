<template>
	<view class="chat-page">
		...
		<view class="bar">
			<image @click="back" style="position: absolute;left: 24rpx;" src="../../static/image/page-previous.png">
			</image> {{$t('home.groupName')}} {{groupType}} USDT
			<view style="position: absolute;right: 24rpx;" v-show="!hasStart" @click="startRed">
				{{$t('home.groupPlay')}} </view>
		</view>
			...
			<view class="flex-column-start" v-if="msgList.length" v-for="(item, index) in msgList" :key="index">
				...
				<view v-if="item.msgType == '红包'|| item.msgType == '普通'">
					...
					<view v-if="item.my" class="flex justify-end padding-right one-show  align-start  padding-top">
								...
								<view class="margin-left padding-chat bg-user-orang"
									...
									<view v-if="item.msgType == '红包'" class="red" style="background: ;">
										<view class="red-envelope" style="background: #c69965;">
											<image style="width: 80rpx;height: 80rpx;" :src="item.content.packetImage">
											</image>
											<view>
												<text>Had get \n</text>
						...
						<view>
							<image :src="item.userAvatar" :text="user.face ? '' : user.name"
								style="width: 80rpx;height: 80rpx;border-radius: 15rpx;" bg-color="#DDDDDD"></image>
						</view>
					</view>
					...
					<view v-else class="flex-row-start margin-left margin-top one-show">
						<view class="chat-img flex-row-center">
							...
							<image mode="aspectFill"
								...
								:src="item.userAvatar" :text="item.userName ? '' : item.name" bg-color="#DDDDDD">
							</image>
						</view>
						<view class="flex" style="width: 500rpx;">
							<view>
								...
								<view class="margin-left padding-chat flex-column-start bg-to-color"
									...
									<!-- 等待领取 用户点击 -->
									<view v-show="item.msgType == '红包' && item.content.status=='等待领取'" class="red"
										@click="handleMessageClick(item)">
										<view class="red-envelope">
											<view
												style="display: block;width: 100%; text-align: end;color: #FEDE8B;font-weight: bolder;">
												{{item.content.amount}} USDT </view>
											<view
												style="display: block;width: 100%; text-align: end; font-size: 24rpx;">
												{{$t('home.groupRedRemaining')}}({{item.content.nowReceiveCount}}/{{item.content.canReceiveCount}})
											</view>
									...
									<view v-show="item.msgType == '红包'&& item.content.status!='等待领取'" class="redInfo">
										<view class="red-envelope" style="background: white; color: #333;">
											<image style="height: 80rpx; width: 80rpx;"
												src="../../static/image/billing.png"></image>
										...
										<view
											style="border-bottom: 1rpx #eee solid;border-top: 1rpx #eee solid;padding: 12rpx 0;display: flex;flex-direction: column;">
											<view v-for="(detail, index) in item.content.packetDetails" :key="index"
												style="display: flex; justify-content: space-between;">
												<text style="color: red;">{{ detail.getterName }}</text>
												<text style="color: red;">{{$t('home.groupRedGet')}}
													{{ detail.getterAmount }} </text>
											</view>
										</view>
									</view>
									<!-- 领取完成 显示详情 -->
									<!-- 普通消息体 -->
									<view v-show="item.msgType == '普通'"
										style="font-size: 32rpx;color:#999;margin: 24rpx;text-align: center;">
										{{item.content}}
									</view>
									<!-- 普通消息体 -->
								</view>
		...
		<view :style="{position: 'fixed',bottom:inputHeight+'px'}" class="flex-column-center"
			...
			<view class="bottom-dh-char flex-row-around" style="font-size: 55rpx;">
				...
				<view @click="sendMessage" class="cu-tag bg-main-color send"
					style="font-size: 32rpx;background: #44c31b;border-radius: 12rpx;">
					{{$t('home.groupSend')}}
				</view>
			</view>
		</view>
		...
		<view class="red-envelope-modal" v-show="showRedEnvelopeModal">
			<view class="red-envelope-content" @click="receiveRedEnvelope">
				...
				<view style="font-size: 68rpx;margin-top: 250rpx;">{{currentItem.content.amount}}<label
						style="font-size: 32rpx;">USDT</label></view>
			</view>
			<image @click="closeRedEnvelopeModal" style="width: 68rpx;height: 68rpx;margin-top: 24rpx;"
				src="../../static/image/close.png"></image>
		</view>
		...
		<view class="fixed-buttons">
			...
			<view>
				<image @click="openCustomerService" src="../../static/image/service.png"></image>
				{{$t('home.groupService')}}
			</view>
		</view>
...
<style scoped>
	...
	.red {
		...
		width: 450rpx;
		font-size: 32rpx;
		background-image: url('../../static/img/red-bg.png');
